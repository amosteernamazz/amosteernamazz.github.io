 ...
 * 静态初始化在程序加载的过程中完成。
 * 包括全局变量初始化和const类型的初始化
   * zero initialization 的变量会被保存在 bss 段
   * const initialization 的变量则放在 data 段内
   * 其次全局类对象也是在编译器初始化。
 ...
 **静态局部变量的构造和析构**
  ...
  * 对于局部静态变量，构造和析构都取决于程序的执行顺序
    * 一方面是因为程序的实际执行路径不可预知的
    * 另一方面是因为局部静态变量分布在程序代码各处，彼此直接没有明显的关联，很容易让开发者忽略它们之间的这种关系
 **建议**
  * 减少使用局部静态变量
